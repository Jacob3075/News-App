...
public class ArticleRepository {
    ...
    public void getNewsBySource(String sources) {
        articleAPI
                .getNewsFromSource(API_KEY, sources, LANGUAGE_ENGLISH, PAGE_SIZE, 0)
                .enqueue(
                        new Callback<MediaStackResponse>() {
                            @Override
                            public void onResponse(
                                    @NonNull Call<MediaStackResponse> call,
                                    @NonNull Response<MediaStackResponse> response) {
                            ...
                            public void onFailure(
                                    @NonNull Call<MediaStackResponse> call, @NonNull Throwable t) {
    ...
    public void getNewsByCategory(String category) {
        articleAPI
                .getNewsFromCategory(API_KEY, category, LANGUAGE_ENGLISH, PAGE_SIZE, 0)
                .enqueue(
                        new Callback<MediaStackResponse>() {
                            @Override
                            public void onResponse(
    ...
    public void getNewsByKeyWord(String keyWord) {
        articleAPI
                .getNewsByKeyWords(API_KEY, keyWord, LANGUAGE_ENGLISH, PAGE_SIZE, 0)
                .enqueue(
                        new Callback<MediaStackResponse>() {
                            @Override
                            public void onResponse(
